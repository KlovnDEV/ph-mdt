...
RegisterNetEvent("QBCore:Client:SetDuty", function(state, job)
    if not job then job = QBCore.Functions.GetPlayerData().job.name end

    if AllowedJob(job) then
        TriggerServerEvent("ps-mdt:server:ToggleDuty", state)
    end
end)
...
RegisterCommand('mdt', function()
    ...
    if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
        if AllowedJob(PlayerData.job.name) ~= nil then
            TriggerServerEvent('mdt:server:openMDT', activeSubdivision)
        end
    else
...
RegisterNetEvent('mdt:client:dashboardWarrants', function()
    print("getting dashboard warranst")
    TriggerServerEvent("mdt:server:getAllWarrants", true)
...
RegisterNetEvent('mdt:client:open', function(bulletin, activeUnits, calls, cid)
    EnableGUI(true)
    SendNUIMessage({ type = "data", activeUnits = activeUnits, citizenid = cid, ondutyonly = Config.OnlyShowOnDuty, name = "Welcome, " ..PlayerData.job.grade.name..' '..PlayerData.charinfo.lastname:sub(1,1):upper()..PlayerData.charinfo.lastname:sub(2), fullname = PlayerData.charinfo.firstname..' '..PlayerData.charinfo.lastname, bulletin = bulletin })
    SendNUIMessage({ type = "calls", data = calls })
    TriggerEvent("mdt:client:dashboardWarrants")
end)

RegisterNetEvent('mdt:client:open-public', function()
    EnableGUI(true)
    TriggerEvent("mdt:client:dashboardWarrants")
end)
...
RegisterNUICallback("searchWarrants", function(data, cb)
    ...
    TriggerServerEvent('mdt:server:searchWarrants', warrant)
    cb(true)
end)
...
RegisterNUICallback("newwarrantSearchPerson", function(data, cb)
    local name = data.name
    TriggerServerEvent('mdt:server:newwarrantSearchPerson', name )
    cb(true)
end)
...
RegisterNetEvent('mdt:client:getWarrants', function(sentData)
...
RegisterNetEvent('mdt:client:newwarrantSearchPerson', function(sentData)
    SendNUIMessage({ type = "newwarrantSearchPerson", data = sentData })
end)
...
RegisterNUICallback('expungePerson', function(data, cb)
    TriggerServerEvent('ps-mdt:server:expungePerson', data.cid)
    cb(true)
end)
...
RegisterNUICallback("getAllWarrants", function(data, cb)
    TriggerServerEvent('mdt:server:getAllWarrants', false)
    cb(true)
end)
RegisterNUICallback("getActiveLawyers", function(data, cb)
    TriggerServerEvent('mdt:server:getActiveLawyers')
    cb(true)
end)
RegisterNUICallback("getPrisoners", function(data, cb)
    TriggerServerEvent('mdt:server:getPrisoners')
    cb(true)
end)

RegisterNUICallback("getWarrantData", function(data, cb)
    local warrantId = data.warrantId
    print(" get data for ", warrantId)
    TriggerServerEvent('mdt:server:getWarrantData', warrantId)
    cb(true)
end)

RegisterNUICallback("removeWarrant", function(data, cb)
    local warrantId = data.warrantId
    TriggerServerEvent('mdt:server:removeWarrant', warrantId)
    cb(true)
end)
...
RegisterNUICallback("getAllWeapons", function(data, cb)
    TriggerServerEvent('mdt:server:getAllWeapons')
    cb(true)
end)

RegisterNUICallback("getWeaponData", function(data, cb)
    local serialNumber = data.serialNum
    TriggerServerEvent('mdt:server:getWeaponData', serialNumber)
    cb(true)
end)

RegisterNUICallback("getWeaponEvent", function(data, cb)
    local id = data.id
    TriggerServerEvent('mdt:server:getWeaponEvent', id)
    cb(true)
end)

RegisterNUICallback("searchWeapons", function(data, cb)
    local name = data.name
    TriggerServerEvent('mdt:server:searchWeapons', name)
    cb(true)
end)

RegisterNUICallback("newWeaponRegEvent", function(data, cb)
    local existing = data.existing
    local id = data.id
    local title = data.title
    local serialnumber = data.serialNum
    ...
    local tags = data.tags
    --local time = data.time
    TriggerServerEvent('mdt:server:newWeaponRegEvent', existing, id, title, serialnumber, details, tags)
    cb(true)
end)
...
RegisterNUICallback("newWarrant", function(data, cb)
    ...
    local cid = data.personCID
    local details = data.warrantDet
    local duration = data.duration
    local incidentNum = data.incNum
    local reportNum = data.repNum
    --local time = data.time
    TriggerServerEvent('mdt:server:newWarrant', cid, incidentNum, reportNum, duration, details, warrantId)
    cb(true)
end)

RegisterNUICallback("callPerson", function(data, cb)
    print("call ", data.number)
    TriggerEvent('qb-phone:client:callNumber', data.number, data.name, false)
    cb(true)
end)


RegisterNetEvent('mdt:client:getAllWarrants', function(sentData)
    SendNUIMessage({ type = "allWarrants", data = sentData })
end)
RegisterNetEvent('mdt:client:getAllWarrantsDashboard', function(sentData)
    SendNUIMessage({ type = "allWarrantsDashboard", data = sentData })
end)

RegisterNetEvent('mdt:client:getAllWeapons', function(sentData)
    SendNUIMessage({ type = "weapons", data = sentData })
end)

RegisterNetEvent('mdt:client:getWeaponData', function(sentData)
    SendNUIMessage({ type = "weaponsData", data = sentData })
end)
RegisterNetEvent('mdt:client:getWeaponEventData', function(sentData)
    SendNUIMessage({ type = "weaponsEventData", data = sentData })
end)

RegisterNetEvent('mdt:client:weaponsComplete', function(sentData)
    SendNUIMessage({ type = "weaponsComplete", data = sentData })
end)

RegisterNetEvent('mdt:client:warrantsComplete', function(sentData)
    SendNUIMessage({ type = "warrantsComplete", data = sentData })
end)

RegisterNetEvent('mdt:client:returnWarrantData', function(sentData)
    SendNUIMessage({ type = "warrantData", data = sentData })
end)
RegisterNetEvent('mdt:client:returnAllLawyers', function(sentlawyers, sentjudges)
    SendNUIMessage({ type = "lawyersData", lawyers = sentlawyers, judges = sentjudges })
end)
RegisterNetEvent('mdt:client:returnAllPrisoners', function(sentPrisoners, sentProbations)
    SendNUIMessage({ type = "prisonersData", prisoners = sentPrisoners, probations = sentProbations})
end)
...
--[[
  if isPolice(PlayerData.job.name) and impound.impoundChanged == true then
            ...
            for k,v in pairs(vehicles) do
                ...
                if plt == plate then
                    ...
                    if dist < 5.0 then
                        ...
                        TriggerServerEvent('mdt:server:saveVehicleInfo', dbid, plate, imageurl, notes, stolen, code5, impound, strikes)
                    end
                    break
                end
            end
...
RegisterNUICallback("sendReplyMessage", function(data, cb)
    callid = data.cid
    message = data.replyMessage
    --transfer callid to target id
    QBCore.Functions.TriggerCallback("mdt:server:getCallSource", function(result)
        if result then
            TriggerServerEvent('dispatch:server:911r', message, tonumber(result))
        end
    end, callid)
...
RegisterNUICallback("setSubdivision", function(data, cb)
    activeSubdivision = data.newsubdivision
    cb(true)
end)
...
RegisterNUICallback("deleteIncident", function(data, cb)
    TriggerServerEvent('mdt:server:deleteIncident', data.cid, data.incidentId)
    cb(true)
end)

RegisterNUICallback("deleteReport", function(data, cb)
    TriggerServerEvent('mdt:server:deleteReport', data.cid, data.reportId)
    cb(true)
end)

RegisterNUICallback("trackPerson", function(data, cb)
    TriggerServerEvent('mdt:server:trackPerson', data.personCID)
    cb(true)
end)

RegisterNUICallback("endProbation", function(data, cb)
    TriggerServerEvent('mdt:server:endProbation', data.personCID)
    cb(true)
end)
...
RegisterNetEvent('mdt:client:getVehicleData', function(sentData)
    if sentData and sentData[1] then
        ...
        vehicle['dataModelName'] = vehicle['vehicle']
...
RegisterNUICallback("dispatchMessage", function(data, cb)
    ...
    TriggerServerEvent('dispatch:server:311', data.message) --on the nui callback (client->nui), after sending message to server, send message to dispatch too
    cb(true)
end)

RegisterNUICallback("refreshDispatchMsgs", function(data, cb)
    TriggerServerEvent('mdt:server:refreshDispatchMsgs', GetPlayerServerId(PlayerId()))
    cb(true)
end)
...
RegisterNUICallback("removeImpound", function(data, cb)
    TriggerServerEvent('mdt:server:removeImpound', data)
    cb('ok')
end)
...
RegisterNetEvent('mdt:client:setWaypointCoord', function(coords)
