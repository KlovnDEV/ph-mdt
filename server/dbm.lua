...
function GetPersonInformation(cid)
	local result = MySQL.query.await('SELECT information, tags, gallery, pfp, fingerprint FROM mdt_data WHERE cid = ?', { cid})
	return result[1]
...
function GetConvictionsWithExpungement(cids)
    local result = MySQL.query.await('SELECT latestexpungement FROM mdt_data WHERE cid = ?', { cid })
	local expungement = result[1]
	return MySQL.query.await('SELECT * FROM `mdt_convictions` WHERE `cid` IN(:sentCids) AND `time` > :sentExpungementTime', { 
        sentCids = cids,
        sentExpungementTime = expungement 
    })
	-- return exports.oxmysql:executeSync('SELECT * FROM `mdt_convictions` WHERE `cid` IN(?)', { cids })
end
...
function GetVehicleInformation(plate, cb)
    local result = MySQL.query.await('SELECT * FROM `mdt_vehicleinfo` WHERE plate=:plate LIMIT1;', { plate = plate})
	cb(result)
end
